...
Built on DummyJSON, treating its product catalogue as the clinic's stock catalogue.
Live app: http://savannah-clinic-inventory.vercel.app/ 
...
Sign out button
Category dropdown(decided to go with skeletal loading design instead)
Item Detail View
...
The app is used on a variety of devices, most prominently tablets, so a simple, clean design works best - properly spaced, with colour used intentionally. Also on-screen navigation is necessary because button navigation on basic android tablets can be frustrating.
...
Robust: Semantic HTML (<nav>, <button>, <label>, heading order) so screen readers parse it. Readable and usable at 360px wide.]
...
Deployment & CI/CD
Hosting: Deployed on Vercel. Public URL: https://savannah-clinic-inventory.vercel.app . Deploy branch: main. Every merge into main triggers an automatic production deploy on Vercel. A vercel.json rewrite serves index.html for all routes so client-side deep links (e.g. /items/5) resolve correctly instead of 404ing on the host.
...
What did you use AI for, per section (design / build / deploy / reflection)?
In the design to help me brainstorm and articulate my ideas for example. When brainstorming my thoughts began to drift into distributed systems but AI reminded me that we are using dummy data and a mockAPI
During the build to write repetitive code and help with functions especially syntax in .ts files.
During deployment to helpe me trouble shoot Vercel errors
During reflection to make sure what I wrote is up to par. It did not give me any content just helped me ensure everything made sense. 
Which tools? Did you use a spec-driven or agent workflow framework? If not, how did you structure the work?
The tool I used was Claude I did not use agentic or spec-driven framework. I first designed the project on paper then created a mind map of how I thought I needed ot tackle it. Then through a combination of Google, Youtube and conversations with Claude slowly pieced together the things on my list. Refreshed on a lot of concepts and learnt a few along the way.
One example where an AI suggestion improved your work - and what you prompted.
I came across a problem with was how do I stop a slow old search from overwriting newer results. AI suggested debouncing the output which solved that issue
One example where AI output was wrong/incomplete/subtly bad, and how you caught it.
When generating repetitive form content the AI got the keyed inputs focus wrong. So when emulating on a tablet after each keyboard click you had to select the field again. I had to fix that through trial and error.
Two decisions you made without AI, and why you trusted your own judgment.
I spotted in the question that the plan is to roll out to other clinics so thought of having a clinicID in order to build for the future. That ended up being one of my learning points in that DummyJSON doesn’t include clinic data so decided not to add it.
...
One part of the codebase you'd struggle to defend, and why.
The optimistic-update rollback logic for updating a stock value logic was a bit complicated. I followed a tutorial but didn’t fully understand. Plan to continue studying this and other scenarios like this I encountered during the project.
